...
checkColnames <- function(userSpecifiedColnames = NULL){
  ...
  if(!is.null(userSpecifiedColnames)){
    ...
    if(!all(c("PPM", "CLASS", "C_CHAIN", "DOUBLE_BOND", "OH_GROUP",
      ...
      stop("ERROR: The userSpecifiedColnames.csv does not have correctly written reference column names (First line of the file). Please create a new userSpecifiedColnames.csv template file in the lipidQuan GUI Global Option procedure tab.")
    }
