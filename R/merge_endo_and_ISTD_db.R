...
#' @description This function merges the two components of the reference
#' database together: the endogene lipid database and the ISTD lipid database.
...
merge_endo_and_ISTD_db <- function(endogene_lipid_db, ISTD_lipid_db){
  ...
  # create database for merging containing all unique colnames from endogene &
  # ISTD db
  database <- unique(colnames(endogene_lipid_db),colnames(ISTD_lipid_db))

  # find unique colnames for endogene & ISTD db
  endogene_lipid_db_unique <- setdiff(colnames(endogene_lipid_db),
                              colnames(ISTD_lipid_db))
  ISTD_lipid_db_unique <- setdiff(colnames(ISTD_lipid_db),
                              colnames(endogene_lipid_db))

  # give endogene & ISTD database all their unique colnames, so that they have
  # the same dimensions (values set to NA)
