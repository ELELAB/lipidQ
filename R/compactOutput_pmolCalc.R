...
#' NAME, CLASS_PMOL_SUBT_PMOL_*, and MOL_PCT_CLASS_SUBT_PMOL_* columns
...
compactOutput_pmolCalc <- function(data, userSpecifiedColnames = NULL){
  ...
  classPmol_molPctClass <- data.frame(x = data[,dataColnames$SUM_COMPOSITION],
  ...
  for(i in 1:nrow(data)){
    classPmol_molPctClass[i, dataColnames$SUM_COMPOSITION] <-
      unlist(strsplit(classPmol_molPctClass[i,dataColnames$SUM_COMPOSITION],
                      " "))[[1]]
  }

  # insert CLASS_PMOL_SUBT_PMOL_*, and MOL_PCT_CLASS_SUBT_PMOL_* columns
  classPmol_molPctClass <- cbind( classPmol_molPctClass,
    data[,c(colnames(data)[grep("^CLASS_PMOL_SUBT_PMOL_",colnames(data))])] )
  classPmol_molPctClass <- cbind( classPmol_molPctClass,
    data[,c(colnames(data)[grep("^MOL_PCT_CLASS_SUBT_PMOL_",colnames(data))])] )



  #### sum filtered values for each classes for each sample after BLNK
  #### subtraction

  # find all unique class names (without numbers)
  exData <- data[-grep("is",data[,dataColnames$SUM_COMPOSITION]),]
  classNames <- gsub("^(\\w+.)[[:space:]].*", "\\1",data[1:nrow(exData),
                dataColnames$SUM_COMPOSITION])
  classNames <-unique(classNames)

  FILTERED_names <- colnames(data)[grep("^FILTERED",colnames(data))]

  # store all sumClassValue to be used later in mol% class calculation
  sumClassValueList <- matrix(numeric(), nrow = length(classNames),
                              ncol = length(FILTERED_names))
  for(i in 1:length(classNames)){
    ...
    for(j in 1:length(FILTERED_names)){
      ...
      sumClassValue <- sum(data[grep(paste0("^",classNames[i]," "),
          data[,dataColnames$SUM_COMPOSITION]),FILTERED_names[j]],na.rm = TRUE)
      ...
      data[grep(paste0("^",classNames[i]," "),
          ...
          paste0("CLASS_",FILTERED_names[j])] <- sumClassValue
    }
  }


  # Combine ^CLASS_PMOL_SUBT_PMOL_ , ^MOL_PCT_CLASS_SUBT_PMOL_ & ^CLASS_FILTERED
  # together
  classPmol_molPctClass <- cbind( classPmol_molPctClass,
              data[,c(colnames(data)[grep("^CLASS_FILTERED",colnames(data))])] )

  # remove rows containing duplicates of SUM_COMPOSITION
  classPmol_molPctClass <- classPmol_molPctClass[!duplicated(
    classPmol_molPctClass[,dataColnames$SUM_COMPOSITION]),]
