...
#Introduction
This tutorial will cover all the basic steps involved in lipidQuan
...
LipidQuan can also be open by going to:
```{r, echo=FALSE}
...
After opening LipidQuan, the operation tab will start at the Quantification tab.
...
#QC plots
LipidQuan offers the possibility to create quality control (QC) plots both
...
"folder-name" as Pathname'. Go back to LipidQuan and paste the folder path
...
LipidQuan and paste the full path (Ctrl + V) into the output folder field.
Remember to change every "\\" in the path to "/". 

After this is done, press "Run Analysis" to start the quantification.

#Results
After quantification, LipidQuan will write "Quantification done!" below the "Run
