...
#Introduction
The tutorial will cover all the basic steps involved in lipidQuan lipidomics analysis from loading the input data to interpreting the final analysis results.

#Data
The data for the analysis will consist of the following files:
...
After opening LipidQuan, the operation tab will start at the Quantification tab. In this tab section, start by choosing the input file to be used by clicking on "Browse ..." under "Choose the input data sets (.csv-files)". For this tutorial we will be using the following files 

mE531_NEG_HighMZ-out.csv

mE531_POS_Chol-out.csv

mE531_POS_LowMZ-out.csv

mE531_NEG_LowMZ-out.csv

mE531_POS_HighMZ-out.csv

These files are located in the following folder:  
```{r, echo=FALSE}
system.file("extdata/mE531_Data_LipidX_Out/", package = "lipidQuan")
```
Go to this folder and select these 5 files and click "Open". As the folder name indicates, in this case these input data files are generated from LipidX.
